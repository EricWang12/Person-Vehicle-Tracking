...
def parse(args):
    ...
    parser_infer.add_argument('--save-images', action='store_true' )
    ...
    parser_infer.add_argument("--deepsort_config", type=str,
...
def worker(rank, args, world, model, state):
    ...
    if args.command == 'infer':
        ...
        infer.infer(model, args.images, args.output, args.resize, args.max_size, args.batch,args.deepsort_config,
            original_annotations=args.annotations, mixed_precision=not args.full_precision,
            is_master=(rank == 0), world=world, use_dali=args.with_dali, verbose=False, save_images = args.save_images, output_path = args.images + '-results/')
